      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div className="bg-hero-gradient px-6 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-bold text-lg">Sync Preferences</h2>
                <p className="text-white/70 text-xs">Customize what gets imported</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-sm font-semibold text-gray-800">How far back to sync?</h3>
            </div>
            <div className="grid grid-cols-3 gap-2">
              {DAY_OPTIONS.map((opt) => (
                ...
                >
                  <span className={`text-sm font-semibold ${syncPeriod === opt.value ? 'text-brand-700' : 'text-gray-700'}`}>
                    {opt.label}
                  </span>
                  <span className={`text-xs mt-0.5 ${syncPeriod === opt.value ? 'text-brand-500' : 'text-gray-400'}`}>
                    {opt.sublabel}
                  </span>
                </button>
              ))}
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-gray-800">Which inboxes to sync?</h3>
              </div>
              <button
                onClick={() =>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className={`text-sm font-medium ${selected ? 'text-brand-700' : 'text-gray-700'}`}>
                        {cat.label}
                      </div>
        ...
        <div className="px-6 pb-5">
          <button
            ...
            className="w-full flex items-center justify-center gap-2.5 bg-white border-2 border-gray-200 hover:border-brand-300 hover:bg-brand-50 text-gray-700 py-3 rounded-xl text-sm font-semibold transition-all shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? (
